...
import { ArrowLeft, Bell, BellOff, TrendingUp, TrendingDown, Minus, Star, MapPin, Clock, ThumbsUp, Edit3, ExternalLink, Trophy } from 'lucide-react'
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      setProducto(data); setCargandoProducto(false)
    }
    cargar()
  }, [id])

  if (cargandoProducto) {
    return (<div className="container-app max-w-4xl py-8"><div className="skeleton h-8 w-48 mb-4" /><div className="skeleton h-32 mb-4" /><div className="skeleton h-24 mb-2" /><div className="skeleton h-24 mb-2" /></div>)
  }
  if (!producto) {
    return (<div className="container-app max-w-4xl py-16 text-center"><div className="text-5xl mb-4">😕</div><h2 className="font-display text-xl font-bold mb-2">Producto no encontrado</h2><button onClick={() => navigate(-1)} className="btn btn-primary mt-4"><ArrowLeft size={16} /> Volver</button></div>)
  }
  ...
  const handleSeguir = async () => { if (estaSiguiendo) { await dejarDeSeguir(id) } else { await seguir(id, ubicacion.ciudad, ubicacion.provincia, precioMin || null) } }

  return (
    <div className="container-app max-w-4xl py-6">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm font-semibold mb-4 transition-colors active:scale-95" style={{ color: 'var(--brand)' }}>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2">
          {/* Product Header */}
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card p-5 mb-4" style={{ boxShadow: 'var(--shadow-md)' }}>
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0" style={{ background: 'var(--brand-glow)' }}>
                {producto.imagen_url ? <img src={producto.imagen_url} alt="" className="w-full h-full object-cover rounded-2xl" /> : catInfo?.emoji || '📦'}
              </div>
              <div className="flex-1">
                <h1 className="text-xl font-bold font-display">{producto.nombre}</h1>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{producto.marca} {producto.presentacion && `· ${producto.presentacion}`}</p>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {producto.nombre_estandarizado && producto.nombre_estandarizado !== producto.nombre && (
                    <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>(también: {producto.nombre_estandarizado})</span>
                  )}
                </div>
              </div>
              {usuario && (
                <button onClick={handleSeguir} className={`btn btn-sm ${estaSiguiendo ? 'btn-primary' : 'btn-secondary'}`} style={{ borderRadius: 'var(--radius-full)' }} title={estaSiguiendo ? 'Dejar de seguir' : 'Seguir producto'}>
          ...
          {precios.length > 1 && ahorro > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="card p-4 mb-4 price-best">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--brand)' }}>💡 Ahorro potencial</p>
                  <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-muted)' }}>Comprando en el más barato vs el más caro</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-extrabold" style={{ color: 'var(--brand)' }}>{fmtPrecio(ahorro)}</p>
                  <p className="text-[10px] font-bold" style={{ color: 'var(--brand-light)', opacity: 0.8 }}>−{ahorroPct}% menos</p>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {precios.length > 0 && (
            <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-2 mb-3">
              <button onClick={() => setCanalFiltro(null)} className="flex-shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all"
                style={!canalFiltro ? { background: 'var(--brand-glow)', borderColor: 'var(--brand)', color: 'var(--brand-dark)' } : { borderColor: 'var(--border)', color: 'var(--text-muted)' }}>Todos</button>
              {CANALES_COMPRA.map(canal => {
                const count = precios.filter(p => p.canal === canal.id).length
                if (count === 0) return null
                return (
                  <button key={canal.id} onClick={() => setCanalFiltro(canalFiltro === canal.id ? null : canal.id)}
                    className="flex-shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all whitespace-nowrap"
                    style={canalFiltro === canal.id ? { background: 'var(--brand-glow)', borderColor: 'var(--brand)', color: 'var(--brand-dark)' } : { borderColor: 'var(--border)', color: 'var(--text-muted)' }}>
                    {canal.emoji} {canal.nombre} ({count})
                  </button>
                )
              })}
              <button onClick={() => setShowMap(!showMap)} className="flex-shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all"
                style={showMap ? { background: 'var(--brand-glow)', borderColor: 'var(--brand)', color: 'var(--brand-dark)' } : { borderColor: 'var(--border)', color: 'var(--text-muted)' }}>📍 Ver mapa</button>
            </div>
          )}
          ...
          <h3 className="text-sm font-bold mb-3 flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
            ...
            <span className="text-[10px] font-normal ml-auto" style={{ color: 'var(--text-muted)' }}>{preciosFiltrados.length} comercio{preciosFiltrados.length !== 1 ? 's' : ''}</span>
            {cargandoPrecios && <div className="w-4 h-4 border-2 rounded-full animate-spin" style={{ borderColor: 'var(--border)', borderTopColor: 'var(--brand)' }} />}
          </h3>
                  ...
                  <motion.div key={p.precio_id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0`}
                          style={{ background: i === 0 ? 'var(--brand-glow-strong)' : 'var(--bg-secondary)' }}>
                          {tipoInfo?.emoji || '🏪'}
                        </div>
                      ...
                      <div className="text-right flex-shrink-0 ml-3">
                        <p className={`text-lg font-extrabold ${i === 0 ? '' : ''}`} style={{ color: i === 0 ? 'var(--brand)' : 'var(--text-primary)' }}>{fmtPrecio(precioEfectivo)}</p>
                    ...
                    {isEditing && (
                      <div className="mt-3 pt-3 border-t animate-fade-in" style={{ borderColor: 'var(--border)' }}>
                        <p className="text-[11px] font-bold mb-2" style={{ color: '#d97706' }}>📢 Solicitar actualización de precio</p>
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => handleSolicitud(p.precio_id)} disabled={guardando || !nuevoPrecioInput} className="btn btn-primary btn-sm">{guardando ? '...' : '✓ Enviar'}</button>
                    ...
                    {!isEditing && (
                      <div className="mt-2.5 pt-2 border-t flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
                        {i === 0 && preciosFiltrados.length > 1 ? (
                          <span className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-1" style={{ color: 'var(--brand)' }}><Trophy size={10} /> Mejor precio</span>
                        ) : (
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => { setEditandoPrecioId(p.precio_id); setNuevoPrecioInput('') }} disabled={!usuario}
                            ...
                            title={!usuario ? 'Iniciá sesión' : ''}>📢 Actualizar</button>
                        </div>
                      </div>
                    )}
                  </motion.div>
          ...
          {usuario && <button onClick={() => navigate('/cargar-precio')} className="w-full mt-5 btn btn-primary btn-lg">🏷️ Cargar precio en otro comercio</button>}
        </div>

        {/* Sidebar */}
        <div className="lg:col-span-1">
          {top3.length > 0 && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="card p-4 mb-4 sticky top-20" style={{ boxShadow: 'var(--shadow-md)' }}>
              <h3 className="text-sm font-bold mb-3 flex items-center gap-2"><Trophy size={14} style={{ color: 'var(--accent)' }} /> Top 3 más baratos</h3>
              <div className="flex flex-col gap-2">
                {top3.map((p, i) => {
                  const precioEfectivo = p.en_oferta && p.precio_oferta ? p.precio_oferta : p.precio
                  return (
                    <div key={p.precio_id} className={`p-3 rounded-xl ${i === 0 ? 'price-best' : ''}`} style={i !== 0 ? { background: 'var(--bg-secondary)' } : {}}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <Link to={`/comercio/${p.comercio_id}`} className="text-xs font-bold truncate block transition-colors"
                        ...
                        <span className={`text-base font-extrabold flex-shrink-0`} style={{ color: i === 0 ? 'var(--brand)' : 'var(--text-primary)' }}>{fmtPrecio(precioEfectivo)}</span>
                      </div>
                    </div>
                  )
                })}
          ...
          {precios.length > 0 && (
            <div className="card p-4" style={{ boxShadow: 'var(--shadow-sm)' }}>
              <h3 className="text-sm font-bold mb-2 flex items-center gap-2">📊 Tendencia de precio</h3>
              <div className="text-center py-4">
                <div className="flex items-center justify-center gap-2 mb-1">
                  {precioMin < precioMax ? <TrendingDown size={20} style={{ color: 'var(--brand)' }} /> : <Minus size={20} style={{ color: 'var(--text-muted)' }} />}
                  <span className="text-2xl font-extrabold" style={{ color: 'var(--brand)' }}>{fmtPrecio(precioMin)}</span>
                </div>
                <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Precio más bajo actual en {ubicacion.ciudad}</p>
                <div className="flex items-center justify-center gap-4 mt-3 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
                  <div><p className="text-xs font-bold">{fmtPrecio(precioMin)}</p><p className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Mínimo</p></div>
                  <div className="w-px h-6" style={{ background: 'var(--border)' }} />
                  <div><p className="text-xs font-bold">{fmtPrecio(precioMax)}</p><p className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Máximo</p></div>
                  <div className="w-px h-6" style={{ background: 'var(--border)' }} />
                  <div><p className="text-xs font-bold">{preciosFiltrados.length}</p><p className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Comercios</p></div>
                </div>
              </div>
            </div>
          )}
